 ...
 *
 * English:
 * this list is ordered and contains all bytes but '\0' '\r' \n'
 *
 * Python3:
 * vorbid = set(b"\0\r\n")
 * enc253 = [x for x in range(256) if not x in vorbid]
 *
 */
var enc253 =
 ...
 *
 * English:
 * this list is the inverse of enc253
 * dec253[enc253[i]]=i for all i in range(0-253)
 * dec253[x]=-1 for all x in { '\0', '\r', '\n' }
 *
 * Python3:
 * dec253 = [-1 for x in range(256)]
 * for (i,v) in zip(enc253,range(253)): dec253[i]=v
 *
 */
var dec253 =
[-1, 0, 1, 2, 3, 4, 5, 6, 7, 8, -1, 9, 10, -1, 11,
...
Encoder.prototype.encode = function(){
	if(this.peek(7)<125){
		this.push(enc253[this.peek(8)]);
		this.rem(8);
	}else{
		this.push(enc253[this.peek(7)]);
		this.rem(7);
	}
};
...
//var enc = new Encoder(function(it){ console.log('>',String.fromCharCode(it)); });
//var enc = new Encoder(function(it){ console.log('>',it); });
